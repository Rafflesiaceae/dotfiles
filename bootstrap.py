...
def log_syschange(did, undo):
    """Log do/undo shell commands to a file as to what this script changes
    on the system - best effort"""
    global __log_cache
...
def log_syschange_write():
    """We don't write logs immediately in order to reverse them at the
    end, so that .undo fields can be dumped/applied without much
    thought"""
    global __log_cache
...
def backup_file(file_path):
    ...
    COUNT_MAX_BACKUPS = 9
    for bak_file_path in (
        "{}.bk{}".format(file_path, i) for i in range(0, COUNT_MAX_BACKUPS)
    ):
        if not pathlib.exists(bak_file_path):
            os.rename(file_path, bak_file_path)
            log_syschange(
                f'mv "{file_path}" "{bak_file_path}"',
                f'mv "{bak_file_path}" "{file_path}"',
            )
            return

    raise Exception(
        "reached maximal number ({}) of allowed backups for file {}".format(
            COUNT_MAX_BACKUPS, file_path
        )
    )
...
def is_already_linked(source_path, target_path):
    ...
    if common.isWindows:
        ...
        return pathlib.realpath(source_path) == pathlib.realpath(
            common.sh_cygpath(common.sh_realpath(target_path))
        )
...
def jinja_paths(repo_path, raw_source_path, raw_target_path):
    ...
    source_path = pathlib.join(repo_path, "build") + pathlib.realpath(
        raw_source_path
    ).replace(repo_path, "")
    ...
    if pathlib.splitext(raw_target_path)[1] == ".j2":
        target_path = repo_path + pathlib.realpath(raw_source_path).replace(
            repo_path, ""
        )
...
def resolve_sources_list(sources):
    ...
    sources = {k: empty_v_to_k(k, resolve_source_obj(v)) for (k, v) in sources.items()}
...
def main():
    ...
    # TODO this might be obsolete now, we change from blacklisting to whitelisting
    ignores = load_ignore_file()

    config = JinjaGlobalsModule(common.load_config())
    global home_base_path
    home_base_path = (
        config.home_base_path if hasattr(config, "home_base_path") else home_base_path
    )

    sources = resolve_sources_list(list.sources)
    for source_path, target_path in sources.items():
        # set target_path to be local of $HOME
        target_path = pathlib.abspath(home_path_equivalent(repo_path, target_path))
        ...
        if pathlib.splitext(source_path)[1] == ".j2":
            ...
            # request confirmation in the case that he output of a
            # template has been changed, such changes will be lost and
            # should be migrated to the actual template file
            # XXX I know this is a mess, just stick with it for now

            if (
                pathlib.exists(p.source_path)
                and pathlib.exists(p.source_path_bk)
                and not filecmp.cmp(p.source_path, p.source_path_bk)
            ):
                response_should_skip = False
                while True:
                    response = input(
                        '"{}" has been changed since last invocation.\n(D)iscard changes/(S)kip file altogether/(V)iew differences (d/s/v) '.format(
                            p.source_path
                        )
                    )
                    ...
                    if response == "d":
                        ...
                        subprocess.check_call(
                            ["vimdiff", p.source_path, p.source_path_bk]
                        )
